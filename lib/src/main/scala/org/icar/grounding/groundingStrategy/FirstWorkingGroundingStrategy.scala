...
class FirstWorkingGroundingStrategy extends GroundingStrategy {
  ...
  override def apply(inputCapabilities: List[ConcreteCapability]): ConcreteCapability = {
    val notAvailableCapabilities = notWorkingCapabilities.map(x => x.toString).toList.mkString(";")
    if (notAvailableCapabilities.nonEmpty)
      println(s"Not available capabilities: ${notAvailableCapabilities}")
